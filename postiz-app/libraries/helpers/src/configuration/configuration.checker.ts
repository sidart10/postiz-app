...
export class ConfigurationChecker {
  ...
  checkStorageConfiguration() {
    ...
    if (!storageProvider) {
      return; // Already checked by checkNonEmpty
    }

    switch (storageProvider) {
      case 'local':
        if (!this.checkNonEmpty('UPLOAD_DIRECTORY', 'Required for local storage provider.')) {
          this.issues.push(
            'When using local storage (STORAGE_PROVIDER=local), you must set UPLOAD_DIRECTORY to a valid path (e.g., /uploads or ./uploads)'
          );
        }
        break;
      ...
      case 'cloudflare':
        ...
        const missingVars = requiredCloudflareVars.filter(
          (varName) => !this.get(varName)
        );

        if (missingVars.length > 0) {
          this.issues.push(
            `When using Cloudflare R2 storage (STORAGE_PROVIDER=cloudflare), you must set: ${missingVars.join(', ')}`
          );
        }
        break;

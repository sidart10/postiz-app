      ...
      await Promise.all(
        postDetails.flatMap((p) =>
          p?.media?.flatMap(async (m) => {
            const uploadResult = await this.runInConcurrent(
              async () =>
            ...
            // Extract media ID string from upload result
            const mediaId = typeof uploadResult === 'string'
              ? uploadResult
              : uploadResult?.media_id_string || uploadResult?.media_id || String(uploadResult);

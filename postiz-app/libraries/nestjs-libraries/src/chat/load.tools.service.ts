...
export class LoadToolsService {
  ...
  async agent() {
    ...
    return new Agent({
      ...
      - When the user provides images in the 'images' array, you MUST map them to the 'attachments' field in postsAndComments
      - The internal tool uses 'attachments' not 'images' - always use 'attachments' when calling integrationSchedulePostTool
      - Images must be publicly accessible URLs that can be downloaded via HTTP/HTTPS
      - Each image URL in the images array should be added to the attachments array in postsAndComments
      ${renderArray(
        [

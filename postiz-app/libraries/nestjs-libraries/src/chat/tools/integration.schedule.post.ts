...
`,
      ...
      execute: async (args, options) => {
        ...
        // Get storage instance for URL downloads
        ...
        for (const post of context.socialPost) {
          ...
          // Process posts: download external URLs and re-upload to Postiz storage
          const processedValue = await Promise.all(
            post.postsAndComments.map(async (postOrComment) => {
              // Download and re-upload each attachment
              const processedImages = await Promise.all(
                postOrComment.attachments.map(async (url) => {
                  try {
                    console.log(`[IntegrationSchedulePostTool] Downloading image: ${url}`);

                    // Download external URL and upload to Postiz storage
                    const postizUrl = await storage.uploadSimple(url);

                    console.log(`[IntegrationSchedulePostTool] Re-uploaded to: ${postizUrl}`);

                    return {
                      id: makeId(10),
                      path: postizUrl,  // Now it's a Postiz-hosted URL!
                    };
                  } catch (error) {
                    console.error(
                      `[IntegrationSchedulePostTool] Failed to download/upload ${url}:`,
                      error
                    );
                    throw new Error(
                      `Failed to process image ${url}: ${(error as Error).message}`
                    );
                  }
                })
              );
